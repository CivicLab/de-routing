define([
        ...
        'values/language',
        'helpers/Utils'
], function($,_,CONSTANTS,LANGUAGE, Utils){
	...
	var RecordUploader = function(record) {
		...
		this.sendRecordData = function(successCallback,errorCallback) {
			...
			request = $.ajax({
				url: CONSTANTS.SETTING_WEB_SERVICE_URL+"/records/submit",
				type: "post",
				dataType: "json",
			...
			function loopFileArray(arr) {
				...
				self._ckeckFile(arr[fileIndex].uri, function(result) {
					...
					if (result) {
						...
						self._uploadFile(arr[fileIndex],record.get('id'),
							function() { // on success
								fileIndex++;
		...
		this._uploadFile = function(file,record_id,onSuccess,onFail,progressCallback) {
            ...
            //start transfer
            transfer.upload(file.uri, CONSTANTS.SETTING_WEB_SERVICE_URL+"/records/upload" , success, fail, options);
